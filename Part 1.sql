...
-- Since caregivers may benefit from the ability monitor multiple patients at once, so to allocate their attention where it is most needed, and to provide a visualization that fits well on a small screen, I would recommend a clustered heatmap as one data visualization option.
